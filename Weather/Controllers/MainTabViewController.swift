...
class MainTabViewController: UITabBarController {
    ...
    private func setupTabBarControllers() {
        guard let forecastController = R.storyboard
                                        .forecast()
                                        .instantiateInitialViewController(creator: { [weak self] coder in
                  guard let self = self else { return nil }
                  return ForecastViewController(coder: coder,
                                                locationService: self.locationService,
                                                apiManager: self.apiManager)
              }),
              let weatherController = R.storyboard
                                       .weather()
                                       .instantiateInitialViewController(creator: { [weak self] coder in
                  guard let self = self else { return nil }
